...
**Dynamic Embedded Parameters From Storefront Overview**In some cases, it's not enough to define your request parameters statically in the APIEase admin. You may need to send information from your storefront that is only available at runtime, based on the customer's session, selections, or activity.
...
Dynamic embedded parameters allow you to tailor each API request to the individual visitor, cart, or page context. Here are a few common use cases:
